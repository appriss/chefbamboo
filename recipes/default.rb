...
if node[:bamboo][:agent][:enable_ssl]
  ...
  file "/etc/profile.d/bamboo.sh" do
    content "export SSL_OPTS=\"-Djavax.net.ssl.keyStore=#{node[:bamboo][:home]}/bamboo_server.ks -Djavax.net.ssl.keyStorePassword=#{node[:bamboo][:agent][:enable_ssl][:keystore_password]}\";export JAVA_OPTS=$JAVA_OPTS:$SSL_OPTS;"
    owner "root"
    group "root"
    mode "0644"
    action :create
